#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Build and publish the standalone FreeSWITCH + mod_audio_stream + Piper TTS
# image to Docker Hub.
#
# It reuses run.sh to build the image (same FreeSWITCH / mod_audio_stream / Piper
# build-args), tags it with a versioned tag AND :latest, then pushes both.
# Optionally syncs the README to the Docker Hub repo page if `docker-pushrm`
# is installed.
#
# Usage:
...
#   MOD_AUDIO_STREAM_REF=v1.0.0 FS_VERSION=v1.10.11 ./publish.sh
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail

DOCKER_NAMESPACE="${DOCKER_NAMESPACE:-mullayam06}"
IMAGE_NAME="${IMAGE_NAME:-freeswitch-audiostream-piper}"
VERSION="${VERSION:-1.10-audiostream-amy}"
...
# 2) Build the image with the versioned tag (run.sh handles the build-args).
if [[ "${DO_BUILD}" == "1" ]]; then
