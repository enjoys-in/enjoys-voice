...
class MediaServer extends Emitter {
  constructor(conn, mrf, listenAddress, listenPort, advertisedAddress, advertisedPort, profile) {
    ...
    server.on('connection', (sock) => {
      console.log(`[MRF-DIAG] raw TCP connection from ${sock.remoteAddress}:${sock.remotePort}`);
      sock.on('error', (e) => console.log(`[MRF-DIAG] socket error: ${e.message}`));
      sock.on('close', () => console.log(`[MRF-DIAG] socket closed`));
    });
  ...
  */
  connectCaller(req, res, opts, callback) {
    ...
    const __x = async(callback) => {
      ...
      console.log(`[MRF-DIAG] connectCaller: requiresDtls=${requiresDtlsHandshake(req.body)}`);
      if (!requiresDtlsHandshake(req.body)) {
        ...
        console.log(`[MRF-DIAG] connectCaller DTLS path, uuid=${uuid}, uri=${uri}`);
        ...
        this.srf.createUAC(uri, {
          ...
          if (err) {
            console.log(`[MRF-DIAG] createUAC error: ${err.message}`);
          ...
          const obj = this.pendingConnections.get(uuid);
          obj.dialog = dlg;
          console.log(`[MRF-DIAG] createUAC OK, obj.conn=${!!obj.conn}`);
        ...
        const produceEndpoint = (dialog, conn) => {
          console.log(`[MRF-DIAG] produceEndpoint called, dialog=${!!dialog}, conn=${!!conn}`);
